...
Featuring a 4.0-inch ILI9488 TFT display (480×320 resolution) driven over a 60 MHz SPI bus using **LovyanGFX**, Dashboard++ blends real-time multi-sensor fusion algorithms, custom sub-pixel anti-aliased GFX graphics, dirty-rendering optimizations, dynamic CPU frequency scaling, persistent NVS configuration management, and a complete single-page Web application served over WiFi with REST API control and ArduinoOTA updates.
...
- **Sub-Pixel Anti-Aliased Rendering:** Custom vector graphics engine implementing anti-aliased primitives (lines, circles, corner arcs, rounded filled rectangles) with alpha-blending math to eliminate visual pixelation on high-density TFTs.
- **Dirty-Rendering Frame Pipeline:** Double-buffered element state tracking ensuring only mutated visual regions are drawn to the SPI bus, achieving up to 60 FPS frame rates.
- **Configurable 7-Segment Digital Fonts:** 50 pt and 15 pt seven-segment fonts with background "ghost digit" rendering (888 backdrop effect) and fully user-configurable integer and decimal digit boundaries for all telemetry counters.
- **Full Sensor Suite Integration:** Precise fuel level monitoring (20-point piecewise linear calibration table + EMA filtering), engine coolant thermistor telemetry (Steinhart-Hart equation), battery voltage divider monitoring, 3-axis I2C digital compass heading, and trip fuel economy tracking.
...
- **Embedded Web Management Portal:** Embedded single-page Web application accessible over SoftAP or local WiFi network featuring live performance telemetry graphs, interactive slider/color controls, NVS backup/restore, web serial terminal stream, and HTTP file upload OTA firmware updating.
...
│ │  • Speed Fusion & Odometer Distance Calculation   │ │ │  • Sub-Pixel Anti-Aliased  │ │
│ │  • Fuel Economy & Acceleration Timer Engine       │ │ │    Primitives & Alpha Blend  │ │
│ │  • GPS Epoch Time Sync → settimeofday()           │ │ │  • Dynamic CPU Frequency   │ │
│ │  • Safe Thread Sync via g_stateMutex              │ │ │    Scaling (80/160/240 MHz) │ │
│ └───────────────────────────────────────────────────┘ │ │  • Auto Night-Mode Backlight│ │
│ ┌───────────────────────────────────────────────────┐ │ │    PWM Control             │ │
│ │ Task: WebTaskCore0 (Priority 1, Stack 8KB)        │ │ │  • Ignition Sense Deep     │ │
│ │  • SoftAP ("Dashboard_Config") & Multi-SSID STA   │ │ │    Sleep State Machine     │ │
│ │  • WebServer HTTP Handlers (8 REST Endpoints)     │ │ │  • Telemetry Logging to    │ │
│ │  • ArduinoOTA Firmware Listener                   │ │ │    4KB Ring Buffer         │ │
...
| **GPIO18** | `SPI_CLK` | SPI Clock | Output | Hardware SPI SCK pin (Configurable up to 60–80 MHz) |
...
Graphics rendering is built on `LovyanGFX` with specialized antialiasing routines designed to render clean UI elements on high-density displays.

#### Alpha Blending Math
Sub-pixel anti-aliased primitives draw edges with fractional coverage $\alpha \in [0, 1]$. Color blending calculates RGB565 values via 8-bit channel isolation:
$$R_{\text{out}} = \lfloor R_{\text{fg}} \cdot \alpha + R_{\text{bg}} \cdot (1 - \alpha) \rfloor$$
$$G_{\text{out}} = \lfloor G_{\text{fg}} \cdot \alpha + G_{\text{bg}} \cdot (1 - \alpha) \rfloor$$
$$B_{\text{out}} = \lfloor B_{\text{fg}} \cdot \alpha + B_{\text{bg}} \cdot (1 - \alpha) \rfloor$$

#### Ghost-Digit Rendering
The main speed display uses custom 7-segment digital font bitmaps (`DS_DIGIT50pt7b`). Before rendering active numeric glyphs, a dimmed "ghost digit" pattern (`888`) is drawn in `ghost_color` (default `#000000` or custom hex). Active digits overlay this background, simulating physical liquid crystal display segments.
...
Measures time taken to accelerate between configured speed thresholds (`ACCEL_START_SPEED` to `ACCEL_TARGET_SPEED`, e.g., 0–50 km/h or 0–100 km/h) using a 3-state state machine:
...
```
  ...
  └─────────┘                            └─────────┘
       ▲                                      │
       │ Speed < Start (Reset)                │ Speed >= TargetSpeed
       └──────────────────────────────────────┼───────────────────────┐
                                              ▼                       │
                                        ┌──────────┐                  │
                                        │ FINISHED │ <────────────────┘
...
#### Thermal Throttling & Dynamic Scaling Matrix
...
```
                     ...
                     └─── YES ──> Check FPS & Thermal Limits:
                                    ├── Boot (t < 5s): 240 MHz (WiFi Init)
                                    ├── FPS < 50% Target: 240 MHz
                                    ├── FPS < 80% Target: 160 MHz
                                    └── FPS >= 80% Target: 80 MHz
                                    │
...
Dashboard++ uses a generic 3-mode macro system (`processConfig()`) to load, serialize, and deserialize over 50 configuration variables from ESP32 NVS flash storage.

### Key Configuration Categories

#### System & Performance
- `TARGET_FPS` (default=60): Desired display refresh rate.
- `SPI_BUS_SPEED` (default=60000000): SPI bus frequency in Hz.
- `ENABLE_DYNAMIC_CPU` (default=false): Toggles automatic CPU frequency scaling.
...
- `GHOST_COLOR_STR` (default="#000000"): Hex color code for inactive 7-segment digit background.
...
- `SPEED_DIGITS`, `SAT_DIGITS`, `TMR_INT_DIGITS`, `TMR_DEC_DIGITS`, `BAT_INT_DIGITS`, `BAT_DEC_DIGITS`, `INST_INT_DIGITS`, `INST_DEC_DIGITS`, `AVG_INT_DIGITS`, `AVG_DEC_DIGITS`, `FUEL_INT_DIGITS`, `FUEL_DEC_DIGITS`, `ODO_INT_DIGITS`, `ODO_DEC_DIGITS`: Configurable integer and decimal digit limits for all UI numerical readouts.

#### UI Layout Offset Coordinates
- `BIG_CENTER_X`, `BIG_CENTER_Y`: Screen anchor origin point.
- `OFFSET_BIG_TIME_X/Y`, `OFFSET_BIG_DATE_X/Y`, `OFFSET_BIG_SPEED_NUM_X/Y`, `OFFSET_BIG_SPEED_UNIT_X/Y`, `OFFSET_BIG_ODO_X/Y`, `OFFSET_BIG_SAT_X/Y`, `OFFSET_BIG_TMR_X/Y`, `OFFSET_BIG_BAT_X/Y`, `SIDEBAR_LEFT_X/Y`, `SIDEBAR_RIGHT_X/Y`: Fine-grained pixel coordinate offsets for every UI component.
...
├── include/
│   ├── Conthrax_SemiBold12pt7b.h # Vector typeface (Medium header size)
│   ├── Conthrax_SemiBold7pt7b.h  # Vector typeface (Small badge size)
│   ├── Conthrax_SemiBold4pt7b.h  # Vector typeface (Micro label size)
│   ├── DS_DIGIT50pt7b.h          # Custom 50 pt 7-segment digital font bitmap
│   └── DS_DIGIT15pt7b.h          # Custom 15 pt 7-segment digital font bitmap
├── lib/
...
└── src/
    ...
    ├── gfx.cpp            # LovyanGFX display device class, AA primitives, vector icons
...
2. Scroll to the **System Actions** panel.
3. Click **Choose File**, select the `.pio/build/esp32dev/firmware.bin` file, and click **Upload Firmware**.
...
- Engine temperature, fuel level, battery voltage, compass heading, satellite count, and instant/avg KM/L simulate active riding telemetry.
