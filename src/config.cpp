...
int OFFSET_BIG_DATE_X = 118;
int OFFSET_BIG_DATE_Y = -78;
int OFFSET_BIG_SIGNATURE_X = 0;
int OFFSET_BIG_SIGNATURE_Y = -65;
int OFFSET_BIG_SPEED_NUM_X = -7;
int OFFSET_BIG_SPEED_NUM_Y = 18;
int OFFSET_BIG_SPEED_UNIT_X = 134;
int OFFSET_BIG_SPEED_UNIT_Y = 74;
int OFFSET_BIG_ODO_X = 15;
int OFFSET_BIG_ODO_Y = 155;
int OFFSET_BIG_SAT_X = -180;
int OFFSET_BIG_SAT_Y = -78;
int OFFSET_BIG_TMR_X = -53;
int OFFSET_BIG_TMR_Y = -35;
int OFFSET_BIG_BAT_X = -112;
int OFFSET_BIG_BAT_Y = 149;

int SIDEBAR_LEFT_X = 10;
int SIDEBAR_LEFT_Y = 64;
int SIDEBAR_RIGHT_X = 462;
int SIDEBAR_RIGHT_Y = 64;
int SIDEBAR_BAR_WIDTH = 8;
int SIDEBAR_BAR_HEIGHT = 250;

int OFFSET_HALL_ICON_X = 0;
int OFFSET_HALL_ICON_Y = -90;
int OFFSET_WIFI_ICON_X = 195;
int OFFSET_WIFI_ICON_Y = -106;
int OFFSET_INST_KML_X = 60;
int OFFSET_INST_KML_Y = -13;
int OFFSET_AVG_KML_X = 160;
int OFFSET_AVG_KML_Y = -13;
int OFFSET_AVG_SPEED_X = -163;
int OFFSET_AVG_SPEED_Y = -11;
int OFFSET_COMPASS_X = 0;
int OFFSET_COMPASS_Y = -130;
int OFFSET_FUEL_LTRS_X = 132;
int OFFSET_FUEL_LTRS_Y = 150;
...
int OFFSET_BIG_FPS_X = 0;
int OFFSET_BIG_FPS_Y = 0;
...
int REFRESH_SAT_MS = 1000;
int REFRESH_TMR_MS = 10;
...
int TARGET_FPS = 60;
int BACKLIGHT_BRIGHTNESS = 22;
bool ENABLE_AUTO_BRIGHTNESS = false;
int LIGHT_SENSOR_DARK_VAL = 0;
int LIGHT_SENSOR_BRIGHT_VAL = 3685;
int AUTO_BRIGHT_DARK = 20;
...
void processConfig(int mode, JsonDocument *doc) {
  ...
  CFG_INT(BACKLIGHT_BRIGHTNESS, "BL_BRIGHT", 22);
  CFG_BOOL(ENABLE_AUTO_BRIGHTNESS, "EN_AUTO_BL", false);
  CFG_INT(LIGHT_SENSOR_DARK_VAL, "LIGHT_DARK", 0);
  CFG_INT(LIGHT_SENSOR_BRIGHT_VAL, "LIGHT_BRIGHT", 3685);
  CFG_INT(AUTO_BRIGHT_DARK, "AB_DARK", 20);
  ...
  CFG_STR(GHOST_COLOR_STR, "GHOST_C", "#666666");
  ...
  CFG_INT(OFFSET_BIG_TIME_X, "O_BTIME_X", -98);
  CFG_INT(OFFSET_BIG_TIME_Y, "O_BTIME_Y", -78);
  CFG_INT(OFFSET_BIG_DATE_X, "O_BDATE_X", 118);
  CFG_INT(OFFSET_BIG_DATE_Y, "O_BDATE_Y", -78);
  CFG_INT(OFFSET_BIG_SIGNATURE_X, "O_BSIG_X", 0);
  CFG_INT(OFFSET_BIG_SIGNATURE_Y, "O_BSIG_Y", -65);
  CFG_INT(OFFSET_BIG_SPEED_NUM_X, "O_BSN_X", -7);
  CFG_INT(OFFSET_BIG_SPEED_NUM_Y, "O_BSN_Y", 18);
  CFG_INT(OFFSET_BIG_SPEED_UNIT_X, "O_BSU_X", 134);
  CFG_INT(OFFSET_BIG_SPEED_UNIT_Y, "O_BSU_Y", 74);
  CFG_INT(OFFSET_BIG_ODO_X, "O_BODO_X", 15);
  CFG_INT(OFFSET_BIG_ODO_Y, "O_BODO_Y", 155);
  CFG_INT(OFFSET_BIG_SAT_X, "O_BSAT_X", -180);
  CFG_INT(OFFSET_BIG_SAT_Y, "O_BSAT_Y", -78);
  CFG_INT(OFFSET_BIG_TMR_X, "O_BTMR_X", -53);
  CFG_INT(OFFSET_BIG_TMR_Y, "O_BTMR_Y", -35);
  CFG_INT(OFFSET_BIG_BAT_X, "O_BBAT_X", -112);
  CFG_INT(OFFSET_BIG_BAT_Y, "O_BBAT_Y", 149);
  CFG_INT(SIDEBAR_LEFT_X, "SBAR_L_X", 10);
  CFG_INT(SIDEBAR_LEFT_Y, "SBAR_L_Y", 64);
  CFG_INT(SIDEBAR_RIGHT_X, "SBAR_R_X", 462);
  CFG_INT(SIDEBAR_RIGHT_Y, "SBAR_R_Y", 64);
  CFG_INT(OFFSET_HALL_ICON_X, "O_HALL_X", 0);
  CFG_INT(OFFSET_HALL_ICON_Y, "O_HALL_Y", -90);
  CFG_INT(OFFSET_WIFI_ICON_X, "O_WIFI_X", 195);
  CFG_INT(OFFSET_WIFI_ICON_Y, "O_WIFI_Y", -106);
  CFG_INT(OFFSET_INST_KML_X, "O_INST_X", 60);
  CFG_INT(OFFSET_INST_KML_Y, "O_INST_Y", -13);
  CFG_INT(OFFSET_AVG_KML_X, "O_AVG_X", 160);
  CFG_INT(OFFSET_AVG_KML_Y, "O_AVG_Y", -13);
  CFG_INT(OFFSET_AVG_SPEED_X, "O_AVG_SPD_X", -163);
  CFG_INT(OFFSET_AVG_SPEED_Y, "O_AVG_SPD_Y", -11);
  ...
  CFG_INT(OFFSET_BIG_FPS_X, "O_FPS_X", 0);
  CFG_INT(OFFSET_BIG_FPS_Y, "O_FPS_Y", 0);

  CFG_INT(REFRESH_SPEED_MS, "R_SPD", 250);
  CFG_INT(REFRESH_SAT_MS, "R_SAT", 1000);
  CFG_INT(REFRESH_TMR_MS, "R_TMR", 10);
  ...
  CFG_STR(WIFI_SSID, "WIFI_SSID", "Xiaomi 15");
  ...
  CFG_STR(WIFI_SSID_1, "WIFI_S1", "D-Link-627F3B");
  CFG_STR(WIFI_SSID_2, "WIFI_S2", "TP-Link_F3EB");
  CFG_STR(WIFI_SSID_3, "WIFI_S3", "");
  CFG_STR(WIFI_SSID_4, "WIFI_S4", "");
  CFG_INT(WIFI_TX_POWER_DBM, "WIFI_TXP", 20);
  CFG_INT(WIFI_RETRY_MODE, "WIFI_RETRY_M", 0);
  ...
  CFG_STR(OTA_CURRENT_VERSION, "OTA_VER", "1.2.6");
